...
async function createFoodNowPlusSubscription(stripe, database, params) {
  ...
  // Subscription with default_incomplete: first invoice stays unpaid until the customer pays with the Payment Element.
  // Stripe API versions from "Basil" onward often omit invoice.payment_intent; the Payment Element uses
  // invoice.confirmation_secret.client_secret instead (same shape as a PaymentIntent client_secret for Elements).
  ...
  if (invoice && invoice.confirmation_secret && typeof invoice.confirmation_secret === 'object') {
    clientSecret = invoice.confirmation_secret.client_secret;
  }

  if (!clientSecret && invoice && invoice.payment_intent) {
    const paymentIntent =
  ...
  if (!clientSecret) {
    throw new Error(
      'Subscription did not return a client secret for the first invoice. Expand confirmation_secret or payment_intent on the invoice.'
    );
  }
